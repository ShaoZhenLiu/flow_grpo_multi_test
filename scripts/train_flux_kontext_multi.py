...
class GenevalPromptImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # image = Image.open(os.path.join(self.dataset, image_path)).convert('RGB')
        item["image"] = [Image.open(os.path.join(self.dataset, image_path)).convert('RGB').resize((512, 512)) for image_path in image_path_ls]
...
def main(_):
    ...
    if accelerator.is_main_process:
        wandb.init(
            project="flow_grpo_flux_kontext_multi_human",
            # mode="disabled"
        )
